                            ...
                            <td data-col="Assigned Staff">
                                @forelse ($instance->assignments as $assignment)
                                    <form method="POST" action="{{ route('admin.service-tracker.toggle-assignment', $assignment) }}" class="d-inline">
                                        @csrf
                                        <button type="submit" class="badge {{ $assignment->completed ? 'bg-success' : 'bg-secondary' }}" style="border:none;cursor:pointer;" title="Click to toggle">
            ...
            <div class="card-view-list">
                @forelse ($instances as $instance)
                    <div class="cv-card">
                        ...
                        <div class="cv-row"><span class="cv-label">Status</span><span class="cv-value">@php($s = $instance->status)<span class="badge {{ $s === 'done' ? 'bg-success' : 'bg-warning text-dark' }}">{{ $instance->statusLabel() }}</span>@if ($instance->assignments->count()) <small class="text-muted">{{ $instance->completionPercent() }}%</small>@endif</span></div>
                        <div class="cv-row"><span class="cv-label">Assigned Staff</span><span class="cv-value">@forelse ($instance->assignments as $assignment)<form method="POST" action="{{ route('admin.service-tracker.toggle-assignment', $assignment) }}" class="d-inline">@csrf<button type="submit" class="badge {{ $assignment->completed ? 'bg-success' : 'bg-secondary' }}" style="border:none;cursor:pointer;" title="Click to toggle">{{ $assignment->staff_name }} {{ $assignment->completed ? '✓' : '○' }}</button></form>@empty<span class="text-muted">—</span>@endforelse</span></div>
